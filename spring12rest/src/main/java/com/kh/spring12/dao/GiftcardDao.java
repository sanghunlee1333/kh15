...
public class GiftcardDao {
	...
	public void connect(int giftcardNo, int attachmentNo) {
		String sql = "insert into giftcard_image ("
	...
	public List<GiftcardDto> selectList() {
		String sql = "select * from giftcard "
					+ "order by giftcard_price asc, giftcard_charge desc, giftcard_no asc";
	...
	// 수정 메소드
	public boolean update(GiftcardDto giftcardDto) {
		String sql = "update giftcard " + "set giftcard_name = ?, giftcard_content = ?, "
									+ "giftcard_charge = ?, giftcard_price = ? "
									+ "where giftcard_no = ?";
		Object[] data = { giftcardDto.getGiftcardName(), giftcardDto.getGiftcardContent(),
							giftcardDto.getGiftcardCharge(), giftcardDto.getGiftcardPrice(),
							giftcardDto.getGiftcardNo()
						};
